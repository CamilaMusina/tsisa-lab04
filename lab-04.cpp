...
double func(double x, double y) {
    return cos(x) * cos(y) * exp(-pow(x, 2) - pow(y, 2));
}

double AvrValue(std::vector<Individ>& population) {
    double res = 0.0;
...
double MaxValue(std::vector<Individ>& population) {
    double maxp = 0.0;
    for (const auto& ind : population) {
        if (ind.fit > maxp)
            maxp = ind.fit;
...
int main()
{
    std::vector<Individ>Population(4);
    ...
    for (int i = 1; i <= n; i++) {
        ...
        std::vector<Individ>new_population(4);
